...
const defaultCountries = COUNTRIES_FR;

function CountrySelect({ name, value, onChange, required = false, defaultCountry = 'MA', placeholder = 'Sélectionner un pays', className = '', countries = defaultCountries }) {
  const options = useMemo(() => {
    const maroc = countries.find(c => c.code === 'MA');
    const others = countries.filter(c => c.code !== 'MA').sort((a, b) => a.name.localeCompare(b.name, 'fr'));
    return maroc ? [maroc, ...others] : others;
  }, [countries]);
...
export const getCountryName = (code) => {
  if (!code) return '';
  const country = COUNTRIES_FR.find(c => c.code === code);
