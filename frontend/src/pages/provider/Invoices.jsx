...
import { Search, Check, X, FileDown, Trash2, Receipt } from 'lucide-react';
...
export default function Invoices() {
  ...
  const tab = new URLSearchParams(location.search).get('tab') === 'avoirs' ? 'avoirs' : 'factures';

  useEffect(() => {
    if (tab === 'factures') fetchFactures();
    else fetchAvoirs();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab, search, status, destType, column, direction]);

  const switchTab = (next) => {
    const params = new URLSearchParams(location.search);
    if (next === 'avoirs') params.set('tab', 'avoirs');
    else params.delete('tab');
    navigate(`/dashboard/factures?${params.toString()}`);
  };
  ...
  const fetchFactures = useCallback(async () => {
    ...
    try {
      const { data } = await api.get('/invoices', {
        params: { search, statut: status, type_destination: destType, page: 1, ...sortParams },
      });
      setFactures(data.data || []);
    } finally {
      setLoading(false);
    }
  }, [search, status, destType, sortParams]);
          ...
          <Card style={{ padding: 16, marginBottom: 16 }}>
            <div className="flex flex-col md:flex-row" style={{ gap: 12, alignItems: 'center' }}>
              <div style={{ position: 'relative', flex: 1 }}>
                <Search size={16} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--color-smoke)' }} />
                <input
                  type="text"
              ...
              <select value={destType} onChange={(e) => setDestType(e.target.value)} className="select" style={{ maxWidth: 220 }}>
              ...
              <table className="table-clean">
                ...
                <tbody>
                  {factures.map((f) => (
                    ...
                    >
                      <td className="font-mono-data" style={{ color: 'var(--color-primary)' }}>{f.numero}</td>
